...
2. **Follow the workflow** - The `spec-writing` skill is preloaded into your context; follow its full dialogue-to-spec process. For grilling sessions, also load `grill-methodology`; whenever domain terms are being pinned down or an ADR is warranted, load `domain-modeling`; for deepening work, load `architecture-review`.
