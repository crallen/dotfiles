...
If specific files are named, focus there. Otherwise, use the diff below as the review target: prefer staged changes when present, fall back to unstaged changes when nothing is staged, and always include any new untracked files. If the diff says `(no pending changes)`, review the full codebase by exploring it directly. If the diff says `REVIEW BLOCKED`, relay the message to the user and stop immediately.
