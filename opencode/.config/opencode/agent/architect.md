...
2. **Load the workflow** - Use the skill tool to load `spec-writing` for the full dialogue-to-spec process. For decisions that need stress-testing, also load `grill-methodology`; for terminology sharpening or ADR-worthy decisions, load `domain-modeling`.
