...
If the diff below says `REVIEW BLOCKED`, relay the message to the user and stop immediately. If it says `(no pending changes)`, both agents should assess the full codebase directly. If specific files or scope are mentioned, pass that focus to both agents.
...
Spawn both subagents simultaneously — do not wait for one before starting the other. Give each the diff above and any scope from the arguments.
