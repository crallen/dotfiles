---
name: grill
description: Relentless one-at-a-time interrogation of a plan against the project's domain model — sharpens terminology, stress-tests decisions, and writes CONTEXT.md and ADRs inline as conclusions crystallize.
---
...
If `CONTEXT-MAP.md` exists, read it first to understand the context landscape, then read the relevant `CONTEXT.md` files. If only a root `CONTEXT.md` exists, read it. If neither exists, that's fine — you'll create one lazily when the first term is resolved.
